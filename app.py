...
class PrefixMiddleware:
    """WSGI middleware to mount the app under a URL prefix (e.g. /myfin).

    This adjusts SCRIPT_NAME and PATH_INFO so that url_for() and redirects
    generated by Flask/Flask-Login include the prefix when the app is served
    from a subpath behind a reverse proxy.
    """
    ...
    def __call__(self, environ, start_response):
        ...
        if path_info.startswith(script_name + "/") or path_info == script_name:
...
app.wsgi_app = PrefixMiddleware(app.wsgi_app, prefix="/myfin")
    ...
    try:
        query = text(f"""
            SELECT 
                period,
                year,
                {"month," if granularity in ['month', 'day'] else ''}
                {"week," if granularity == 'week' else ''}
                {"day, day_of_week," if granularity == 'day' else ''}
                total_in,
                total_out,
                balance,
                transaction_count
                {", period_start, period_end" if granularity != 'day' else ''}
            FROM {view_name}
            ...
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    try:
                        ...
                        print(f"Failed to parse accounting date: {row['Date comptable']}")
                        print(e)
                        continue
                    ...
                    try:
                        ...
                        print(f"Failed to parse value date: {row['Date valeur']}")
                        print(e)
                    ...
                    try:
                        ...
                        print(f"Failed to parse amount: {row['Montant']}")
                        print(e)
                        continue
